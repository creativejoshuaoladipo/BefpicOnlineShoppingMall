...
namespace BefpicOnlineshoppingMall.Repository
{
    ...
    {
        //Declaring the DBContext values and the DBSet Values
        private dbMyOnlineShoppingEntities _DBEntity;

        DbSet<Tbl_Entity> _dbSet;

        //Dependency Injection of DBEntity in the Constructor of the Class
        ...
        {
            //Checking Where the Record is and storing it in a location before removing it
           Tbl_Entity entity = _dbSet.Where(wherePredict).FirstOrDefault();
            Remove(entity);
        }

        //This is just like removing a list of Products similar to the above that is just a single entity
        public void RemoveRangeBywhereClause(Expression<Func<Tbl_Entity, bool>> wherePredict)
        {
            //Check where the List is and store it in a List having the Datatype of the Class
